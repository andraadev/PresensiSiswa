...
@section('content')
    <div class="row mt-3">
        <div class="col-4">
            ...
            <div class="panel profile-card">
                <div class="card-body d-flex flex-column align-items-center text-center gap-3 p-4">
                    <div class="rounded-circle bg-primary text-white d-flex align-items-center justify-content-center fw-bold fs-1"
                        style="width: 88px; height: 88px; flex: 0 0 auto;">
                        {{ $initials }}
                    </div>
                    <div>
                        <div class="fw-semibold fs-5">{{ $data_user->nama_lengkap }}</div>
        ...
        <div class="col-8">
            <form action="{{ route('data-user.update', $data_user->id) }}" method="POST" class="panel needs-validation">
                @csrf
                @method('PUT')
                @if (in_array($data_user->role, ['Admin', 'BK']))
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('username') is-invalid @enderror" id="username"
                            name="username" value="{{ old('username', $data_user->username) }}">
                        @error('username')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
